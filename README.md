...
- ⚠️ 该程序由 Stoentag000 在 Xiaomi MiMo Claw 的支持下开发
...
| 功能 | 说明 |
|------|------|
| 摄像头扫码 | 实时检测 QR 码 / 条形码，基于 Vision 框架 |
| 多摄像头切换 | 支持内置摄像头、外接 USB 摄像头、iPhone Continuity Camera |
| 图片识别 | 支持 PNG / JPG / TIFF / GIF / BMP / WebP，批量识别图中所有码 |
| 拖拽上传 | 直接拖拽图片到菜单栏面板，自动识别（支持摄像头和图片模式） |
| 自动复制 | 检测到码后自动复制到剪贴板（可关闭，关闭后弹窗提供手动复制按钮） |
| 提示音 | 扫码成功播放系统提示音（可关闭） |
...
1. 运行后菜单栏出现 QR 图标
2. **左键点击** → 打开扫描面板
3. **摄像头模式**：对准 QR 码 → 自动识别
   - 自动复制开启时：结果自动复制到剪贴板
   - 自动复制关闭时：弹窗显示复制按钮，点击即可复制
   - 点击顶部摄像头名称可切换设备（内置 / 外接 / iPhone）
4. **上传图片**：切换模式按钮 → 选择图片，或直接**拖拽图片**到面板上
   - 图片加载失败时会显示错误提示和重新选择按钮
...
└── QRScanner/
    ...
    ├── CameraScanner.swift      # 摄像头采集 + Vision 检测 + 设备管理
...
- **SwiftUI** — 界面框架
- **AVFoundation** — 摄像头采集 + 设备发现（AVCaptureDevice.DiscoverySession）
- **Vision** — QR 码 / 条形码检测（VNDetectBarcodesRequest）
- **NSPopover** — 菜单栏弹出面板
...
| 类型 | 设备 | 说明 |
|------|------|------|
| 内置 | MacBook / iMac 前后置摄像头 | 自动识别，即插即用 |
| 外接 | USB / Thunderbolt 摄像头 | 即插即用 |
| Continuity Camera | iPhone / iPad | 需 macOS Ventura+，同一 Apple ID，设备在附近 |

主界面顶部和设置 → 摄像头均可切换设备。默认为"自动选择"模式，按优先级（内置 → 外接 → Continuity Camera）选取最佳设备。
